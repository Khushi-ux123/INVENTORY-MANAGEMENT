...
export default function Dashboard({ stats, products, customers, orders, onNavigate }: DashboardProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-1">Real-time overview of your store's inventory, clients, and orders.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        <motion.div 
          ...
          className="bg-white border border-neutral-200/80 rounded-2xl p-5 shadow-xs flex items-center justify-between"
        >
          <div className="space-y-1">
            <span className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Total Products</span>
            <div className="text-2xl font-bold text-neutral-900">{stats.totalProducts}</div>
          </div>
        ...
        >
          <div className="space-y-1">
            <span className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Total Customers</span>
        ...
        >
          <div className="space-y-1">
            <span className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Orders Filled</span>
        ...
        <motion.div 
          ...
          className={`border rounded-2xl p-5 shadow-xs flex items-center justify-between ${
            stats.lowStockCount > 0 
              ? "bg-amber-50/50 border-amber-200 cursor-pointer hover:bg-amber-50 transition-colors" 
              : "bg-white border-neutral-200/80"
          }`}
        >
          <div className="space-y-1">
            <span className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Low Stock</span>
            <div className={`text-2xl font-bold ${stats.lowStockCount > 0 ? "text-amber-700" : "text-neutral-900"}`}>
              {stats.lowStockCount}
            </div>
          </div>
          <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${
            stats.lowStockCount > 0 ? "bg-amber-100 text-amber-700" : "bg-neutral-50 text-neutral-400"
          }`}>
        ...
        <motion.div 
          ...
          className="bg-white border border-neutral-200/80 rounded-2xl p-5 shadow-xs flex items-center justify-between sm:col-span-2 lg:col-span-1"
        >
          <div className="space-y-1">
            <span className="text-xs font-medium text-neutral-400 uppercase tracking-wider">Total Revenue</span>
            <div className="text-2xl font-bold text-neutral-900">${stats.totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-neutral-500">Dual-axis visualization of order count & gross revenue</p>
              </div>
              <button 
            ...
            {orders.length > 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <YAxis 
                      ...
                      orientation="right"
                      stroke="#10b981"
                      tick={{ fontSize: 10, fill: '#10b981', fontWeight: 500 }}
                      axisLine={false}
                      tickLine={false}
              ...
              <div className="h-72 border border-dashed border-neutral-200 rounded-xl flex flex-col items-center justify-center text-sm text-neutral-400 p-4">
                <PackageCheck className="h-8 w-8 stroke-1 text-neutral-300 mb-2" />
          ...
          <div className="bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs space-y-4">
            ...
            {lowStockProducts.length > 0 ? (
              <div className="overflow-hidden border border-neutral-100 rounded-xl divide-y divide-neutral-100">
                {lowStockProducts.map(p => (
                  <div key={p.id} className="p-3.5 flex items-center justify-between hover:bg-neutral-50/50 transition-colors">
                    <div className="space-y-0.5">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button 
                        ...
                        className="p-1 px-3 bg-neutral-100 hover:bg-neutral-200 active:scale-95 text-[11px] font-semibold text-neutral-700 rounded-lg transition-all"
                      >
                        Adjust
                      </button>
                    </div>
              ...
              <div className="border border-dashed border-neutral-200 rounded-xl p-6 text-center text-sm text-neutral-400 flex flex-col items-center justify-center">
                    ...
                    {recentOrders.map(order => {
                      ...
                      return (
                        <tr key={order.id} className="hover:bg-neutral-50/40 transition-colors">
                          ...
                          <td className="py-3.5 font-mono text-neutral-500">
                            {totalItems} {totalItems === 1 ? 'item' : 'items'}
                          </td>
                          <td className="py-3.5">
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 border border-emerald-100/60 text-emerald-700">
                              Paid
                            </span>
                          </td>
                          ...
                          <td className="py-3.5 text-right font-mono font-bold text-neutral-800">
        ...
        <div className="space-y-8">
          ...
          {/* Quick Actions Panel */}
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-sm text-white space-y-4">
            <h3 className="text-sm font-semibold tracking-wide uppercase text-neutral-400">Quick Portal</h3>
            <div className="grid grid-cols-1 gap-2.5">
              <button 
                onClick={() => onNavigate("orders")} 
                className="w-full flex items-center justify-between px-4 py-3 bg-white hover:bg-neutral-50 text-neutral-900 font-semibold text-xs rounded-xl transition-all hover:-translate-y-0.5 active:translate-y-0 cursor-pointer shadow-sm"
              >
              ...
              <button 
                onClick={() => onNavigate("products")} 
                className="w-full flex items-center justify-between px-4 py-3 bg-neutral-800 hover:bg-neutral-700/80 text-white font-semibold text-xs rounded-xl transition-all cursor-pointer border border-neutral-700/50"
              >
          ...
          <div className="bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs space-y-4">
            ...
            <p className="text-xs text-neutral-500">Download system database logs in clean CSV format for spreadsheets and reporting.</p>
            <div className="grid grid-cols-1 gap-2">
              <button 
                onClick={() => downloadProductsCSV(products)}
                className="w-full flex items-center justify-between px-3.5 py-2.5 bg-neutral-50 hover:bg-neutral-100 text-neutral-700 font-semibold text-xs rounded-xl transition-all cursor-pointer border border-neutral-200/60"
              >
          ...
          <div className="bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs space-y-4">
            <h3 className="text-sm font-semibold text-neutral-900">Leaderboard (Best Sellers)</h3>
            ...
            {topProducts.length > 0 ? (
                ...
                {topProducts.map((p, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs">
                    <div className="space-y-0.5 max-w-[150px]">
                      <div className="font-semibold text-neutral-800 block truncate">{p.name}</div>
                      <div className="text-neutral-400 font-mono">{p.quantity} items sold</div>
                    </div>
              ...
              <p className="text-xs text-neutral-400">Order statistics will automatically populate here as soon as sales begin.</p>
            )}
          </div>
...
function CustomTooltip({ active, payload, label }: any) {
  if (active && payload && payload.length) {
    return (
      <div className="bg-neutral-900 border border-neutral-800 text-white p-3.5 rounded-xl shadow-xl font-sans text-xs space-y-2 min-w-[160px]">
        <p className="font-semibold text-neutral-300 border-b border-neutral-800 pb-1.5">{label}</p>
